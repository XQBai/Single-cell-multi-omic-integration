...
# ## Compute standard deviation for cells
# Iden_Sphase_cells <- function(scdna_matrix_arm){
#   
#   cells_sd <- apply(scdna_matrix_arm, 2, sd)
#   ## Model the mixture distribution and removing small group
#   estimate_mix_model <- normalmixEM(cells_sd)
#   index_no_Sphase <- which(estimate_mix_model$mu == max(estimate_mix_model$mu))
#   index_label <- apply(estimate_mix_model$posterior, 1, function(x){which(x == max(x))})
#   index_remain <- which(index_label == index_no_Sphase)
#   
#   pdf('./Cell_sd_density_2.pdf')
#   plot(density(cells_sd[index_remain]), main='Density of sd', ylab = 'Density')
#   dev.off()
#   
#   pdf('./density_curve.pdf')
#   mixtools::plot.mixEM(normalmixEM(cells_sd[index_remain]), whichplots = 2)
#   dev.off()
#   
#   # ## Model the rest of variation and filter cells which probability is less than 95% 
#   # cells_posterior <- estimate_mix_model$posterior[ ,index_no_Sphase]
#   # remain_cells_posterior <- cells_posterior[index_remain]
#   # index_remain_final <- index_remain[which(remain_cells_posterior >= 0.95)]
#   # pdf('./density_cells_cluster.pdf')
#   # plot(density(cells_sd[index_remain_final]))
#   # dev.off()
#   return(index_remain)
# }
