...
non_epi_clusters <- epi_scores_summary %>% filter(me < 0.01) %>% filter(mn > ne_cutoff) %>% select(id) %>% .$id %>% as.character()

seu_epi <- subset(seu_merged, idents=non_epi_clusters)
...
saveRDS(seu_epi, 'seurat_nonepi.rds')
...
# Non-epithelial genes
fibro_myof <- c("ACTA2","DCN","SPARC","THY1","LUM","COL1A1","COL14A1","CAV1")
pdf("fibro_markers.pdf")
DoHeatmap(seu_epi, features=fibro_myof) + scale_fill_viridis(option="D")
dev.off()

endothelium <- c("VWF","PECAM1","SELE","SELP","ENG")
pdf("endothelium_markers.pdf")
DoHeatmap(seu_epi, features=endothelium) + scale_fill_viridis(option="D")
dev.off()

dendritic_cells <- c("CD83","IL3RA","CLEC4C","HLA-DQA2","ID2","IRF5")
pdf("dendritic_markers.pdf")
DoHeatmap(seu_epi, features=dendritic_cells) + scale_fill_viridis(option="D")
dev.off()

mast_cells <- c("MS4A2","TPSAB1","CPA3","TPSB2")
pdf("mast_markers.pdf")
DoHeatmap(seu_epi, features=mast_cells) + scale_fill_viridis(option="D")
dev.off()


mono_macro <- c("CD14","FCGR3A","CD163","CD68")
pdf("mono_macro_markers.pdf")
DoHeatmap(seu_epi, features=mono_macro) + scale_fill_viridis(option="D")
dev.off()

pericytes <- c("PDGFRB","RGS5")
pdf("pericytes_markers.pdf")
DoHeatmap(seu_epi, features=pericytes) + scale_fill_viridis(option="D")
dev.off()

seu_t <- subset(seu_epi, idents=c(2, 3, 4, 5, 6, 7, 8, 9, 10))
seu_fibro <- subset(seu_epi, idents = c(1))
saveRDS(seu_t, file = 'seu_t.rds')
saveRDS(seu_fibro, file = 'seu_fibro.rds')
