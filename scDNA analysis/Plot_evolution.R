...
seurat_scDNA_filter <- subset(seurat_scDNA, cells= union(which(celltype == 'normal'), which(celltype == 'tumor')))
...
pdf('phylogeny_no_normal.pdf')
plot.phylo(as.phylo(hc_no), main='Phylogenic Tree', direction = 'downwards',tip.color = mycol_nonormal[sort(hc_no$order)], cex=1.5, edge.width = 2)
dev.off()

pdf('phylogeny_unrooted_no_normal.pdf')
plot.phylo(as.phylo(hc_no), main='Phylogenic Tree', direction = 'downwards', tip.color = mycol_nonormal[sort(hc_no$order)], cex=1.5, edge.width = 2, type = 'unrooted')
dev.off()
