...
label <- as.factor(seurat_scDNA$subclone)
...
# d <- apply(mat, 2, function(x){dist(t(cbind(as.matrix(normal_CNV), x)))[1]})
d <- as.matrix(dist(t(cbind(t(as.matrix(normal_CNV)), mat))))[1, ]
d <- d[1:dim(mat)[2]+1]

mat <- SortCells(mat, label, d)
mat[mat > 6] = 6
labels <- rep(names(table(seurat_scDNA$subclone)), table(seurat_scDNA$subclone))
...
seurat.no.normal <- subset(seurat_scDNA, cells = which(seurat_scDNA$celltype == 'tumor'))
labels.no.normal <- seurat.no.normal$subclone
labels.no.normal <- data.frame(cluster=labels.no.normal)
mat.no.normal <- scdna_matrix_merge[, rownames(labels.no.normal)]
mat.no.normal <- SortCells(mat.no.normal, label[which(seurat_scDNA$celltype == 'tumor')], d[which(seurat_scDNA$celltype == 'tumor')])
mat.no.normal[mat.no.normal > 6] = 6

label.no.normal <- rep(names(table(seurat.no.normal$subclone)), table(seurat.no.normal$subclone))
...
labels.normal <- seurat.normal$subclone
labels.normal <- data.frame(cluster=labels.normal)

labelnormal <- as.factor(seurat.normal$subclone)
...
label.normal <- rep(names(table(seurat.normal$subclone)), table(seurat.normal$subclone))
...
labels.tech <- seurat.tech$subclone
...
labels.noise <- seurat.noise$subclone
labels.noise <- data.frame(cluster=labels.noise)

labelnoise <- as.factor(seurat.noise$subclone)
...
# obj <- Sort_subclones(mat.noise, labels.noise, hc)
# mat.normal <- obj[[1]]
# labels.normal <- obj[[2]]
